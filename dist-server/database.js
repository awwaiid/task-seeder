...
export class Database {
    ...
    async saveBracket(id, bracketData, expiresInDays = 30) {
        if (!this.db)
            throw new Error('Database not initialized');
    ...
    async updateTournamentShareStatus(id, isShared, shareId) {
        if (!this.db)
            throw new Error('Database not initialized');
        const query = `
      UPDATE tournaments 
