...
export class Tournament {
    ...
    if (this.type === 'double') {
      ...
      if (entrants.length <= 3) {
        ...
        try {
          ...
          this.bracket = Array.isArray(doubleElim)
            ? doubleElim
            : doubleElim.bracket || [];
          ...
          console.error(
            'Failed to create double elimination with tournament-pairings, using single elimination fallback:',
            error
          );
          this.type = 'single';
          ...
          // The tournament-pairings library returns the bracket directly, not as a .bracket property
          this.bracket = Array.isArray(singleElim)
            ? singleElim
            : singleElim.bracket || [];
          this._addParticipantsToBracket(entrants);
          this.lossCount = new Map();
  ...
  getRankings(): Participant[] {
    ...
    const remainingWhoPlayed = this.remainingParticipants.filter(p =>
      participantsWhoPlayed.has(p)
    );
    const remainingWhoDidntPlay = this.remainingParticipants.filter(
      p => !participantsWhoPlayed.has(p)
    );
  ...
  rebuildInternalState(): void {
    ...
    this.completedMatches.forEach((match: any) => {
      this.completedMatchesSet.add(
        `${match.round}-${match.matchInRound || match.match}`
      );
    });
  ...
  private _advanceWinnerInBracket(
    match: ActiveMatch,
    winner: Participant,
    ...
    if (originalMatch?.win) {
      const nextMatch = this.bracket.find(
        (m: any) =>
          m.round === originalMatch.win.round &&
          m.match === originalMatch.win.match
      );
      if (nextMatch) {
    ...
    if (this.type === 'double' && originalMatch?.loss && loser) {
      const loserMatch = this.bracket.find(
        (m: any) =>
          m.round === originalMatch.loss.round &&
          m.match === originalMatch.loss.match
      );
  ...
  private _isUntitledParticipant(
    participant: any,
    ...
    if (typeof participant === 'object') {
      ...
      if (
        taskNameColumn &&
        (!participant[taskNameColumn] ||
          participant[taskNameColumn].trim() === '')
      ) {
        return true;
      }
      ...
      const nameFields = ['name', 'title', 'task', 'summary'];
      const hasValidName = nameFields.some(
        field =>
