...
export class StorageOptimizer {
    static getStorageUsage(): StorageUsage {
        try {
            ...
            return { totalBytes: 0, totalKB: 0, totalMB: 0, bracketCount: 0, estimatedLimit: 5 * 1024 * 1024, usagePercent: 0 }
        }
    }
    ...
    static cleanupOldBrackets(keepCount: number = 10): number {
        try {
            const brackets: Record<string, BracketData> = JSON.parse(localStorage.getItem('bracketology_saved_brackets') || '{}')
