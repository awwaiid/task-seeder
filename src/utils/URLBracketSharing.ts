...
export class URLBracketSharing {
    ...
    static expandTournament(simplifiedTournament: SimplifiedTournament): ExpandedTournament {
        return {
    ...
    static createShareableURL(bracketData: BracketData, baseURL: string = window.location.origin + window.location.pathname): string {
    ...
    static updateURLWithBracket(bracketData: BracketData, replaceState: boolean = true): void {
        try {
            const encoded = this.encodeBracketToURL(bracketData)
