...
describe('Storage Optimization', () => {
    it('should significantly reduce storage size with participant indexing', () => {
        ...
        const largeCsvRow = {
            'Task Name': 'Very long task name with lots of details and descriptions that would normally be repeated',
            ...
            'Description': 'This is a very long description field that contains a lot of text and would normally be duplicated many times throughout the tournament structure, causing massive storage bloat when storing match data.',
        ...
        const tournamentState = {
            ...
            matchHistory: new Map([
                [csvData[0], [
                    { round: 1, opponent: csvData[1], result: 'W', matchNumber: 1, bracket: 'winners' }
                ]],
                [csvData[1], [
                    { round: 1, opponent: csvData[0], result: 'L', matchNumber: 1, bracket: 'winners' }
                ]]
            ])
        }
        ...
        console.log(`Optimized size: ${Math.round(optimizedSize / 1024 * 100) / 100}KB`)
        console.log(`Legacy size: ${Math.round(legacySize / 1024 * 100) / 100}KB`)
        console.log(`Space saved: ${Math.round((1 - optimizedSize / legacySize) * 100)}%`)
